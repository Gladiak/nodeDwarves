...
function updateRaidTick(state, config, runtime) {
  ...
  moveRaidBeasts(raidState.beasts || [], runtime);
...
function finishRaid(state, config, raidState) {
  ...
  const deathRate = clamp(lerp(deathMin, deathMax, difficulty) * (1 - defense), 0, 1);
  ...
  const exposedRatio = population > 0 ? exposedCount / population : 0;
  const lossConfig = raidConfig.resourceLoss || {};
  const lossMin = Math.max(0, Number(lossConfig.min ?? 0));
  const lossMax = Math.max(lossMin, Number(lossConfig.max ?? lossMin));
  const baseLoss = lerp(lossMin, lossMax, difficulty);
  const lossRatio = clamp(baseLoss * exposedRatio * (1 - defense * 0.8), 0, 1);
...
function applyRaidResourceLoss(state, lossRatio, weights) {
  ...
  for (const [resource, amount] of Object.entries(stockpile)) {
    ...
    const loss = Math.min(current, Math.floor(current * lossRatio * weight));
    if (loss > 0) {
...
function formatRaidLoot(stolen) {
  const entries = Object.entries(stolen || {}).filter(([, value]) => value > 0);
  if (entries.length === 0) {
    return '';
  }
...
function spawnRaidBeasts(state, runtime, raidConfig) {
  ...
  for (let i = 0; i < count; i += 1) {
    beasts.push(spawnRaidBeast(runtime));
  }
  return beasts;
}

function spawnRaidBeast(runtime) {
  const width = runtime.gridWidth;
  const height = runtime.gridHeight;
  const edge = Math.floor(Math.random() * 4);
  if (edge === 0) {
    return { x: randomBetween(0, width - 1), y: 0 };
  }
  if (edge === 1) {
    return { x: randomBetween(0, width - 1), y: height - 1 };
  }
  if (edge === 2) {
    return { x: 0, y: randomBetween(0, height - 1) };
  }
  return { x: width - 1, y: randomBetween(0, height - 1) };
}

function moveRaidBeasts(beasts, runtime) {
  if (!Array.isArray(beasts)) {
    return;
  }
  for (const beast of beasts) {
    moveDwarf(beast, runtime);
  }
}
  ...
  if (dwarf.homeId) {
    ...
    if (house && house.type === 'house') {
      if (dwarf.x !== house.x || dwarf.y !== house.y) {
        moveTowards(dwarf, { x: house.x, y: house.y }, runtime);
      }
      return true;
    }
  }
